"""speech-eres2netv2w24s4ep4-sv-zh-cn-16k-common SV extraction (npy in -> npy out)."""
...
WORKER_ALIAS = "speech-eres2netv2w24s4ep4-sv-zh-cn-16k-common"

# Single SV worker session — one extract at a time.
_features_lock = asyncio.Lock()


def format_download_name(source_filename: str) -> str:
    """Download filename: same stem as upload (no _sv suffix)."""
...
def validate_waveform_npy(path: Path) -> None:
    ...
    Pipeline B contract: 16 kHz, conventional amplitude in [-1, 1]
    (no 1145.14 scaling — that is pipeline A / HuBERT).
    """
...
def sync_start_session() -> FeaturesModelStateResponseBody:
    ...
    if result.newly_started:
        return FeaturesModelStateResponseBody(
            loaded=True,
            message="speech-eres2netv2w24s4ep4-sv-zh-cn-16k-common worker loaded.",
        )
    return FeaturesModelStateResponseBody(
        loaded=True,
        message="speech-eres2netv2w24s4ep4-sv-zh-cn-16k-common worker was already loaded.",
    )
...
def sync_stop_session() -> FeaturesModelStateResponseBody:
    ...
    if released:
        return FeaturesModelStateResponseBody(
            loaded=False,
            message="speech-eres2netv2w24s4ep4-sv-zh-cn-16k-common worker released.",
        )
    return FeaturesModelStateResponseBody(
        loaded=False,
        message="speech-eres2netv2w24s4ep4-sv-zh-cn-16k-common worker was not loaded.",
    )
...
def sync_extract_npy(
    ...
    session_dir = Path(
        tempfile.mkdtemp(prefix="features-speech-eres2netv2w24s4ep4-sv-zh-cn-16k-common-")
    )
    ...
    download_name = format_download_name(source_filename)
...
async def extract_upload(
    ...
    """Extract SV embedding from uploaded waveform npy."""
